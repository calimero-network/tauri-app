...
fn write_trampoline(macos: &Path, trampoline_src: &Path) -> Result<PathBuf, LauncherError> {
    // A copy of the bundled Mach-O trampoline, which resolves its own bundle's
    // app.json and execs the installed shell. It must be a real binary: macOS 26
    // LaunchServices silently refuses to spawn a bundle whose executable is a
    // script, and a Mach-O also lets LS read the arch support that the old
    // script trampoline had to fake with `arch -$(uname -m)`.
...
/// Ad-hoc sign the assembled bundle; macOS 26 LaunchServices also refuses
/// unsigned bundles. Best-effort: a signing failure downgrades to the old
/// behaviour rather than failing launcher creation.
...
/// Rewrite an existing launcher's trampoline with the CURRENT format (today: the
/// Mach-O executable). A cheap migration for launcher `.app`s generated by an
/// older desktop version - no icon/app-config churn, just the executable, then a
/// re-sign since replacing it invalidates the bundle signature. Idempotent.
