...
// ================================
// Uses OS keychain (Keychain on macOS, Credential Manager on Windows, libsecret on Linux)
// to securely store JWT tokens instead of plaintext localStorage.
...
fn secure_store_token(key: String, value: String) -> Result<(), String> {
    ...
    info!("[SecureStorage] Stored token for key: {}", key);
    Ok(())
}
