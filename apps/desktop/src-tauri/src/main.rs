...
/// Maximum allowed length for node names
const MAX_NODE_NAME_LENGTH: usize = 64;

/// Validates a node name to prevent path traversal and command injection attacks.
///
/// Valid node names must:
/// - Be non-empty
/// - Be at most MAX_NODE_NAME_LENGTH characters
/// - Contain only alphanumeric characters, hyphens (-), and underscores (_)
/// - Not contain path separators (/, \) or other special characters
/// - Not start with a hyphen (to avoid being interpreted as a command flag)
///
/// # Arguments
/// * `node_name` - The node name to validate
///
/// # Returns
/// * `Ok(())` if the node name is valid
...
fn validate_node_name(node_name: &str) -> Result<(), String> {
    ...
    // Check that name doesn't start with a hyphen (could be interpreted as command flag)
    ...
    // Validate each character against whitelist: alphanumeric, hyphen, underscore
    for (i, c) in node_name.chars().enumerate() {
        if !c.is_ascii_alphanumeric() && c != '-' && c != '_' {
            // Provide specific error messages for common attack patterns
            if c == '/' || c == '\\' {
                return Err(format!(
...
fn create_desktop_shortcut(
    ...
    {
        let desktop =
            dirs::desktop_dir().ok_or_else(|| "Could not find Desktop folder".to_string())?;
...
async fn start_merod(
    ...
    // Validate node name early to prevent path traversal and command injection
    // This must happen before any path construction or command execution
    ...
    if let Some(name) = &node_name {
        ...
        if config_path.exists() {
            ...
            let mut config: toml::Value = config_content
                .parse()
    ...
    // Node name required (already validated earlier in the function)
...
async fn init_merod_node(
    ...
    // Validate node name to prevent path traversal and command injection
...
async fn get_merod_logs(
    ...
    // Validate node name to prevent path traversal attacks
    validate_node_name(&node_name)?;
...
mod tests {
    ...
    fn test_valid_node_names() {
        ...
        assert!(validate_node_name("MyNode123").is_ok());
        assert!(validate_node_name("test_node_1").is_ok());
        assert!(validate_node_name("a").is_ok());
        assert!(validate_node_name("A1_b2-c3").is_ok());
    }

    #[test]
    fn test_reject_empty_node_name() {
    ...
    #[test]
    fn test_reject_long_node_name() {
        // Name exactly at the limit should pass
        let max_name = "a".repeat(super::MAX_NODE_NAME_LENGTH);
        assert!(validate_node_name(&max_name).is_ok());

        // Name over the limit should fail
        let too_long = "a".repeat(super::MAX_NODE_NAME_LENGTH + 1);
        assert!(validate_node_name(&too_long).is_err());
        let err = validate_node_name(&too_long).unwrap_err();
    ...
    fn test_reject_path_traversal() {
        ...
        assert!(validate_node_name("..").is_err());
        assert!(validate_node_name("node/../other").is_err());
        assert!(validate_node_name("node/subdir").is_err());
        assert!(validate_node_name("/etc/passwd").is_err());
        assert!(validate_node_name("\\windows\\system32").is_err());

        // Verify error messages mention path traversal
        let err = validate_node_name("../etc").unwrap_err();
        assert!(err.contains("path") || err.contains("separator"));
    }
    ...
    fn test_reject_command_injection() {
        ...
        assert!(validate_node_name("node && cat /etc/passwd").is_err());
        assert!(validate_node_name("node | malicious").is_err());
        assert!(validate_node_name("$(whoami)").is_err());
        assert!(validate_node_name("`whoami`").is_err());
        assert!(validate_node_name("node$HOME").is_err());

        // Verify error messages mention shell/command injection
        let err = validate_node_name("node;rm").unwrap_err();
        assert!(err.contains("shell") || err.contains("metacharacter"));
    }

    #[test]
    fn test_reject_leading_hyphen() {
        // Names starting with hyphen could be interpreted as command flags
        assert!(validate_node_name("-node").is_err());
        assert!(validate_node_name("--node").is_err());
        assert!(validate_node_name("-").is_err());

        let err = validate_node_name("-node").unwrap_err();
        assert!(err.contains("hyphen") || err.contains("flag"));
    }
    ...
    fn test_reject_special_characters() {
        ...
        assert!(validate_node_name("node%20").is_err());
        assert!(validate_node_name("node!").is_err());
        assert!(validate_node_name("node*").is_err());
        assert!(validate_node_name("node?").is_err());
        assert!(validate_node_name("node<>").is_err());
        assert!(validate_node_name("node\"quoted\"").is_err());
        assert!(validate_node_name("node'quoted'").is_err());
    }
    ...
    fn test_reject_unicode_attacks() {
        ...
        assert!(validate_node_name("nodé").is_err());
        assert!(validate_node_name("ноде").is_err()); // Cyrillic
        assert!(validate_node_name("节点").is_err()); // Chinese
        assert!(validate_node_name("node\u{0000}").is_err()); // null byte
