...
export default function AppCard({
  ...
  // (`min_runtime_version`). The type beside this has carried the value under a
  // note that it was worth surfacing rather than storing — this is that.
  //
  // ⚠️ `0.1.0` is the registry's PLACEHOLDER, applied to any bundle published
  // without one, not a runtime anybody ran. Printing it would put a confident
  // wrong answer on most cards, so it reads as absent — the same rule
  // `installSize` and `publishedAt` already follow here.
  //
  // Marketplace only: an INSTALLED app is a different question, and the desktop
  // already answers that one properly by measuring `merod --version` per node
  // (see utils/merodVersions.ts). This is about a bundle you have not installed.
  const runtime =
        ...
        {runtime && (
          <>
            ...
            <span
              ...
              // and until now the only way to learn that was to press Install
              // and read the toast.
