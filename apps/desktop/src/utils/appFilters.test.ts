...
describe("tagFacets", () => {
  ...
  it("never offers a category slug as a keyword chip", () => {
    expect(tagFacets([app("games", ["games", "puzzle"])]).map((f) => f.id)).toEqual([
      "puzzle",
    ]);
  });
});
...
describe("matchesFacets", () => {
  ...
  it("filters to one shelf", () => {
    expect(matchesFacets(app("games"), { ...none, category: "games" })).toBe(true);
    expect(matchesFacets(app("social"), { ...none, category: "games" })).toBe(false);
    expect(matchesFacets(app(undefined), { ...none, category: "games" })).toBe(false);
  });
  ...
  it("ANDs the tags: a second chip narrows, it never widens", () => {
    ...
    expect(matchesFacets(chat, { ...none, tags: ["chat", "voxel"] })).toBe(false);
  });

  it("combines a shelf with a keyword", () => {
    const app1 = app("games", ["multiplayer"]);
    expect(matchesFacets(app1, { category: "games", tags: ["multiplayer"] })).toBe(true);
    expect(matchesFacets(app1, { category: "social", tags: ["multiplayer"] })).toBe(false);
  });
});
