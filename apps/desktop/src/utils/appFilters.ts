 ...
 *
 * Ported from app-registry's Explore page so the three surfaces that list the
 * same bundles slice them the same way. Keep them in step.
 *
 * ⚠️ THE FACETS ARE DERIVED FROM THE LISTING, NEVER FROM `CATEGORIES`. A chip
 * row of ten shelves where seven return nothing reads as a broken filter
 ...
 * the result, which is the only reading under which a row of filters can
 * never surprise you by growing the list.
 */
export function matchesFacets(app: Facetable, selection: FacetSelection): boolean {
